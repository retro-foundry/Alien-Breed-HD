...
typedef struct {
    ...
    uint32_t        *automap_seen_hash;     /* open-addressing table storing wall index+1, 0 = empty */
...
typedef struct {
    ...
    int16_t         cfg_audio_buffer_samples; /* ab3d.ini: SDL callback buffer size in sample frames (0 = backend default) */
...
#define MAX_PENDING_BLASTS 8
    ...
    bool            cfg_show_fps;       /* 1 = draw rounded FPS (ammo digits) top-left; updates ~4/s */
    uint16_t        fps_display;        /* rounded Hz (game_loop updates ~4/s) */
    ...
    /* AB3D2 mouse-look projectile aim speed. Kept here so v5/v6 full saves
